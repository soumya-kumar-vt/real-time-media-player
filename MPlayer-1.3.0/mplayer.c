...
/*************************************************************************************/
/*CHRONOS MODIFICATIONS BEGIN*********************************************************/
#define ENABLE_CHRONOS			0
#if ENABLE_CHRONOS
...
#define FRAME_EXEC_TIME		15000 		//this in microseconds based off of average frame execution time plus a margin

#define SCHED_GLOBAL_MASK 	0x8000
#define SCHED_ALGORITHM 	SCHED_RT_RMA
#define SCHED_CPUS 		4


static void find_job_deadline(long long period, struct timespec *start_time, struct timespec *deadline)
{
	unsigned long long nsec, carry;
	unsigned long long offset = period;	//the offset from the start time this deadline is, in nanoseconds

	nsec = start_time->tv_nsec + offset;
	carry = nsec / BILLION;
...
{
	if (SCHED_ALGORITHM & SCHED_GLOBAL_MASK)
...
static int check_framedrop(double frame_time)
{
    ...
    if (mpctx->sh_video && !mpctx->d_video->eof && mpctx->sh_audio && !mpctx->d_audio->eof) {
...
//==================== START PLAYING =======================
	...
	if(set_scheduler(SCHED_ALGORITHM, get_priority(), SCHED_CPUS))
	{
		printf("Error: Could not set scheduler.\n");
		return 0;
	}
	struct timespec start_time, deadline, period;
	period.tv_sec = 0;
	period.tv_nsec = (BILLION/(mpctx->sh_video->fps));
	...
	//struct timespec start, end;
	    ...
	    find_job_deadline(period.tv_nsec, &start_time, &deadline);

	    begin_rtseg_self(TASK_RUN_PRIO, MILLION, &deadline, &period, FRAME_EXEC_TIME); 
	    #endif
...
//================= Keyboard events, SEEKing ====================
	...
	/*********************************************************/
	// print drop_frame_cnt and total_frame_cnt
	mp_msg(MSGT_CPLAYER, MSGL_STATUS, "\n");
	if(mpctx->sh_video)
		mp_msg(MSGT_CPLAYER, MSGL_STATUS, "Dropped Frames/Total Frames: %d/%d \n", drop_frame_cnt, total_frame_cnt);
	/*********************************************************/
